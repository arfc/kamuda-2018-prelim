...
The confusion matrix for this dataset is shown in Figure \ref{fig:conf_matrix_10s}. Note that the false positive rate increases, but often there was enough information in each spectrum to correctly identify the isotope. This implies that the ANN trained in this way can be effective in non-ideal measurement situations (low count rate source, short measurement time, incorrectly calibrated detector).
...
An ANN was trained to identify bare and shielded isotopes. The ANN used a training library composed of bare and shielded sources from the ANSI N42$-$34$-$2006 library. The $^{152}$Eu isotope emits a wide range of gamma-rays.
